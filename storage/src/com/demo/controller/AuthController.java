...
public class AuthController extends HttpServlet {
    ...
    public void login(HttpServletResponse response, HttpServletRequest request) throws IOException, ServletException {
        ...
        if(user1.getUserType().equals("采购员")){
            request.getSession().setAttribute("loginUser", user1);
            request.getSession().setMaxInactiveInterval(Integer.MAX_VALUE);
            request.getRequestDispatcher("menuBuyer.jsp").forward(request, response);
            return;
        }else if(user1.getUserType().equals("销售员")){
            request.getSession().setAttribute("loginUser", user1);
            request.getSession().setMaxInactiveInterval(Integer.MAX_VALUE);
            request.getRequestDispatcher("menuSale.jsp").forward(request, response);
            return;
        }
